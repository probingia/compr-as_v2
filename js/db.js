...
const DB_VERSION = 1;
const STORES = ['productos', 'categorias', 'tiendas', 'listaAutocompletado', 'config'];
    ...
    request.onupgradeneeded = (event) => {
      ...
      STORES.forEach(storeName => {
        if (!database.objectStoreNames.contains(storeName)) {
            // La mayoría de nuestras tiendas usan un 'id' como clave principal.
            if (['productos', 'categorias', 'tiendas'].includes(storeName)) {
                 database.createObjectStore(storeName, { keyPath: 'id' });
            } else {
                // Para datos que no son una lista de objetos con id (como config o listaAutocompletado)
                // no usamos un keyPath y los datos se guardarán con una clave explícita.
...
export const saveState = async (state) => {
  ...
  // Guardar configuración (ej. tamaño de fuente)
  ...
  return new Promise((resolve, reject) => {
    transaction.oncomplete = () => {
      // console.log('Estado guardado en IndexedDB con éxito.');
      resolve();
    };
...
export const loadState = async () => {
  ...
  const promises = STORES.map(storeName => {
      return new Promise((resolve, reject) => {
          ...
          request.onsuccess = () => {
              // Casos especiales para datos que no son listas de objetos
