import { state, getCategories, getStores, addCategory, editCategory, deleteCategory, addStore, editStore, deleteStore, addProduct, updateProduct, deleteProduct, toggleProductStatus } from './state.js';
...
const abrirModalEdicion = (producto) => {
    ...
    dom.editModalBody.innerHTML = `
        ...
        <div class="mb-3">
            ...
            <input type="text" id="edit-nombre-producto" class="form-control" value="${producto.nombre}">
        </div>
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                <label for="edit-cantidad-producto" class="form-label">Cantidad</label>
                <input type="number" id="edit-cantidad-producto" class="form-control" value="${producto.cantidad}">
            </div>
            <div class="col-md-6">
                <label for="edit-unidad-producto" class="form-label">Unidad</label>
                <select id="edit-unidad-producto" class="form-select">
        ...
        <div class="mb-3">
            <label for="edit-precio-producto" class="form-label">Precio Unitario</label>
            <div class="input-group">
                <input type="number" id="edit-precio-producto" class="form-control" value="${producto.precioUnitario}" step="0.01" min="0">
                <span class="input-group-text">${precioLabel}</span>
            </div>
        </div>
        <div class="mb-3">
            <label for="edit-categoria-producto" class="form-label">Categoría</label>
            <select id="edit-categoria-producto" class="form-select"></select>
        </div>
        <div class="mb-3">
            <label for="edit-tienda-producto" class="form-label">Tienda</label>
            <select id="edit-tienda-producto" class="form-select"></select>
        </div>
        <div class="mb-3">
            <label for="edit-prioridad-producto" class="form-label">Prioridad</label>
            <select id="edit-prioridad-producto" class="form-select">
        ...
        <div class="mb-3">
            <label for="edit-notas-producto" class="form-label">Notas</label>
            <textarea id="edit-notas-producto" class="form-control" rows="2">${producto.notas}</textarea>
        </div>
    `;
    ...
    const tiendaSelect = dom.editModalBody.querySelector('#edit-tienda-producto');
    if (tiendaSelect) {
...
const handleGuardarEdicion = () => {
    ...
    const nombreInput = document.getElementById('edit-nombre-producto').value;
    const cantidadInput = document.getElementById('edit-cantidad-producto').value;
    const precioInput = document.getElementById('edit-precio-producto').value;
    ...
    const productData = {
        ...
        unidad: document.getElementById('edit-unidad-producto').value,
        precioUnitario: validacion.precio,
        categoriaId: document.getElementById('edit-categoria-producto').value,
        tiendaId: document.getElementById('edit-tienda-producto').value,
        prioridad: document.getElementById('edit-prioridad-producto').value,
        notas: document.getElementById('edit-notas-producto').value.trim(),
    };
...
const exportToTxt = (productos) => {
    ...
    productos.forEach(p => {
        ...
        contenido += `[${p.comprado ? 'x' : ' '}] ${p.nombre} (${p.cantidad} ${p.unidad}) ${tiendaNombre} - ${p.precioTotalCalculado.toFixed(2)}€\n`;
    });
...
const handleGuardarNombreEditado = () => {
    ...
    if (type === 'categoria') {
        ...
        const result = editStore(id, nuevoNombre);
        if (result.success && result.changed) {
            renderGestion();
            render();
        }
