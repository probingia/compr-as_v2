...
const crearFilaSeleccionHTML = (p, options) => {
    ...
    const prioridad = p.prioridad ? capitalizar(p.prioridad) : 'Baja';
    const notas = p.notas || '';
...
export const renderGestion = () => {
    ...
    getCategories().sort((a, b) => a.nombre.toLowerCase().localeCompare(b.nombre.toLowerCase()));
    ...
    getStores().sort((a, b) => a.nombre.toLowerCase().localeCompare(b.nombre.toLowerCase()));
    getStores().forEach(t => {
        const item = document.createElement('li');
        item.className = 'list-group-item d-flex justify-content-between align-items-center';
